# LATR sample/sequence filters for key-token tree search.
# Ported from https://github.com/starreeze/latr/blob/main/model/sample_filter.py
...
from verl.workers.rollout.latr.diverge import DivCollator, DivJudge
from verl.workers.rollout.latr.metrics import rouge_l_score, suffix_match_score
from verl.workers.rollout.latr.kt_utils import BranchInfo

math_core_symbols = "+-*/\\<>=()[]{}_^&%$0123456789"


class KeyTokenFilter(ABC):
    """Filters that determine whether new branches should be created at a specific token."""
...
class ProbFilter(KeyTokenFilter):
    """Only keep tokens with probability greater than thresholds."""
...
class SimilarityFilter(KeyTokenFilter):
    """Only keep remaining tokens with similarity to the first token less than threshold."""
